...
(function () {
  ...
  `;
      document
        .getElementById("chatbot-user-info-form")
        .addEventListener("submit", (e) => {
          ...
          if (name && phone && email) {
            this.state.userInfo = { name, phone, email };
            this.state.isUserInfoCollected = true;
            this.initializeChat(); // now start the real chat
          }
        });
    },
    ...
    loadChatbot: async function () {
      try {
        ...
        document.getElementById("chatbot-widget-name").textContent =
    ...
    initializeChat: function () {
      if (!this.state.isUserInfoCollected) {
    ...
    handleOptionClick: function (option) {
      ...
      if (currentQuestion) {
        const followUpQuestion = this.state.questions.find(
          (q) =>
            q.parentQuestionId === this.state.currentQuestionId &&
            q.triggerOption === option
        );
    ...
    renderMessages: function () {
      ...
      const optionBtns = container.querySelectorAll(
        ".chatbot-widget-option-btn"
      );
